...
export async function POST(request: Request) {
  ...
  const { fullName, email, plan, message } = body as Record<string, unknown>;
  ...
  if (!selectedPlan || selectedPlan.length > 100) {
  ...
  const { error } = await resend.emails.send({
    ...
    to: [to],
    replyTo: senderEmail,
    subject: `Plan inquiry: ${selectedPlan}`,
    text: `Name: ${name}\nEmail: ${senderEmail}\nPlan: ${selectedPlan}\n\n${inquiryMessage}`,
    html: `
      <h2>Plan Inquiry</h2>
      <p><strong>Name:</strong> ${escapeHtml(name)}</p>
      <p><strong>Email:</strong> ${escapeHtml(senderEmail)}</p>
      <p><strong>Plan:</strong> ${escapeHtml(selectedPlan)}</p>
